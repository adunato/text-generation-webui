...
# Install wheel package before using bdist_wheel
RUN . /build/venv/bin/activate && pip3 install wheel
...
COPY softprompts /app/softprompts
...
ENV CLI_ARGS=""
CMD . /app/venv/bin/activate && python3 server.py --chat --model-menu --listen --listen-port 7860
#CMD ["tail", "-f", "/dev/null"]
